...
const CostumeCard = (props:{name:string, url:string, costumeImg:string, costumeImg_url:string}) => {
    ...
    return (
            <div className="card" style={{minWidth: '16rem', maxWidth: '17rem', background: 'rgb(220, 53, 69)', margin: 5}}>
                ...
                <div className="card-body">
                    <a className="card-title" href={props.url} target='_blank' rel="noreferrer" style={{textDecoration: 'none', color: 'white', fontWeight: isHover ? 'bold' : 'normal'}} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>{props.name + ' '}<i className="bi bi-bag"></i></a>
                </div>
            </div>
    )
}
